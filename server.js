...
function startWebServer(client) {
    ...
    app.post('/api/v1/activities', express.json({ limit: '100kb' }), async (req, res) => {
        ...
        console.log('[API_KEY_DEBUG] req.headers:', JSON.stringify(req.headers));
        ...
        const validTypes = ['report_submit', 'event_completed', 'partnership_claimed', 'pulse_submission'];
        if (!validTypes.includes(activity_type)) {
            return res.status(400).json({ error: `Bad Request: Invalid activity_type. Allowed types: ${validTypes.join(', ')}` });
        }
        ...
        try {
            ...
            if (!fork) {
                return res.status(404).json({ error: `Not Found: Fork not found for city "${city}".` });
            }

            let points = 0;
            if (activity_type === 'report_submit') {
                points = gamification.POINTS.REPORT_SUBMISSION || 15;
            } else if (activity_type === 'event_completed') {
                points = gamification.POINTS.EVENT_COMPLETED || 50;
            } else if (activity_type === 'partnership_claimed') {
                points = gamification.POINTS.PARTNERSHIP_CLAIMED || 25;
            } else if (activity_type === 'pulse_submission') {
                points = gamification.POINTS.PULSE_SUBMISSION || 2;
            }

            // Award points
            await notion.updateForkPoints(fork.id, points);

            // Send Discord notification
            try {
                const channelId = config.CHANNEL_IDS.teamChat || config.CHANNEL_IDS.announcement;
                const channel = await client.channels.fetch(channelId).catch(() => null);
                if (channel) {
                    const embed = new EmbedBuilder()
                        .setTitle(`📡 Programmatic Activity Logged // ${activity_type.toUpperCase()}`)
                        .setDescription(`An external system registered an activity for the **${city.toUpperCase()}** node.`)
                        .setColor(config.COLORS.success)
                        .addFields(
                            { name: '⌬ NODE', value: `\`${city.toUpperCase()}\``, inline: true },
                            { name: '⌬ ACTIVITY', value: `\`${activity_type.toUpperCase()}\``, inline: true },
                            { name: '⌬ TELEMETRY', value: `\`+${points} PTS\``, inline: true }
                        )
                        .setFooter({ text: config.BRANDING.footerText })
                        .setTimestamp();

                    if (details) {
                        embed.addFields({ name: '⌬ DETAILS', value: typeof details === 'object' ? JSON.stringify(details) : String(details), inline: false });
                    }

                    await channel.send({ embeds: [embed] });
                }
            } catch (discordErr) {
                console.error('[API_ACTIVITIES] Failed to send Discord notification:', discordErr.message);
            }
            ...
            return res.status(200).json({
                ...
                message: `Successfully logged activity and awarded ${points} points to ${city.toUpperCase()} fork.`
            });
