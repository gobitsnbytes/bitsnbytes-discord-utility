...
async function processNext() {
	...
	try {
		...
		const transcriptData = await withTimeout(
			transcribeMeeting(mergedFilePath, {
				title: meeting.title,
		...
		// Clean up remote OCI files (fire-and-forget)
		if (segmentObjectNames && segmentObjectNames.length > 0) {
			try {
				const { deleteFromOCI } = require('./storageClient');
				const cleanupObjects = [...segmentObjectNames, `meetings/${meetingId}/merged_meeting.ogg`];
				console.log(`[PIPELINE] Clean up: Triggering fire-and-forget OCI cleanup for ${cleanupObjects.length} objects...`);
				Promise.allSettled(cleanupObjects.map(obj => deleteFromOCI(obj)))
					.then(results => {
						const successes = results.filter(r => r.status === 'fulfilled').length;
						const failures = results.filter(r => r.status === 'rejected').length;
						console.log(`[PIPELINE] OCI cleanup completed: ${successes} deleted, ${failures} failed`);
					})
					.catch(() => {});
			} catch (cleanupErr) {
				console.warn('[PIPELINE] Failed to initiate OCI cleanup:', cleanupErr.message);
			}
		}
