...
const { uploadSegmentToPAR } = require('./storageClient');

const mergeCallbackEmitter = new EventEmitter();
const meetingDirMap = new Map();

// Helper to formulate full URL from base PAR URL and object path
...
async function mergeAudioSegmentsRemote(userSegments, meetingDir, meetingStartTime, meetingId) {
	...
	// 1. Upload segments to OCI Object Storage via PAR
	const uploadedSegments = [];
	const segmentObjectNames = [];
	
	for (let i = 0; i < allSegments.length; i++) {
		const seg = allSegments[i];
		const objectName = `meetings/${meetingId}/segments/seg_${i}_${seg.userId}.ogg`;
		
		console.log(`[AUDIO_PROCESSOR] Uploading segment ${i + 1}/${allSegments.length} to OCI: ${objectName}`);
		const url = await uploadSegmentToPAR(objectName, seg.file);
		...
		segmentObjectNames.push(objectName);
	}

	// 2. Generate merged upload and download URLs
	const mergedObjectName = `meetings/${meetingId}/merged_meeting.ogg`;
	const mergedUploadUrl = getFullUrl(mergedObjectName);
	const mergedDownloadUrl = mergedUploadUrl;
	...
	const dispatchUrl = 'https://api.github.com/repos/gobitsnbytes/bitsnbytes-discord-utility/dispatches';
	...
	// 5. Store meetingDir in map for callback download lookup
	meetingDirMap.set(meetingId, { meetingDir });

	// 6. Wait for webhook callback via event emitter
	return new Promise((resolve, reject) => {
