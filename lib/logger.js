...
class Logger {
     ...
     */
    async _sendToDiscord({ type, message, details, color, user, command }) {
        ...
        if (!this.client || !this.client.isReady()) {
            this.queue.push({ type, message, details, color, user, command, timestamp });
            return;
        }
    ...
    boot(message, details = null) {
        this._sendToDiscord({ type: 'BOOT', message, details, color: config.COLORS.success });
    }
