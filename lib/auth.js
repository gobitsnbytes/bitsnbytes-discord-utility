...
function isExecutiveLeader(member) {
	if (!member) return false;
	if (member.permissions?.has('Administrator')) return true;
	if (hasRoleName(member, 'Executive Leadership') || hasRoleName(member, 'Executive Leader')) return true;
...
function isDepartmentLead(member) {
	if (!member) return false;
	return hasRoleName(member, 'Department Lead') || hasRoleMatching(member, /department lead$/i);
}
...
function isForkDepartmentLead(member, city) {
	...
	return hasRoleName(member, 'Tech Lead') || 
	       hasRoleName(member, 'Creative Lead') || 
	       hasRoleName(member, 'Ops Lead') || 
		...
		if (isDeptLead) {
			...
			if (action === 'modify') {
				if (extra && extra.track) {
					...
					if (member.roles.cache.some(r => r.name?.toLowerCase() === 'tech lead')) memberLeadRoles.push('tech');
					if (member.roles.cache.some(r => r.name?.toLowerCase() === 'ops lead')) memberLeadRoles.push('ops');
					if (member.roles.cache.some(r => r.name?.toLowerCase() === 'creative lead')) memberLeadRoles.push('creative');
					if (member.roles.cache.some(r => r.name?.toLowerCase() === 'outreach lead')) memberLeadRoles.push('outreach');
...
function getCoreAdminRoles(guild) {
	...
	allRoles.forEach(role => {
		const nameLower = role.name?.toLowerCase();
		if (nameLower === 'department lead' || nameLower?.endsWith(' department lead')) {
			if (!roles.some(r => r.id === role.id)) {
				roles.push(role);
			}
		}
...
function getCoreAdminAndParentRoles(guild) {
	...
	allRoles.forEach(role => {
		const nameLower = role.name?.toLowerCase();
		if (parentTrackNames.includes(nameLower)) {
			if (!roles.some(r => r.id === role.id)) {
				roles.push(role);
			}
		}
