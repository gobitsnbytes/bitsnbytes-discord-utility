...
// Configure connection timeouts for long-running Gemini requests
try {
	const { Agent, setGlobalDispatcher } = require('undici');
	setGlobalDispatcher(new Agent({
		headersTimeout: 600_000, // 10 minutes
		bodyTimeout: 1200_000,    // 20 minutes
		keepAliveTimeout: 60_000,
		connections: 20,
	}));
	console.log('[TRANSCRIBER] Global undici dispatcher configured with 10m/20m timeouts');
} catch (err) {
	console.warn('[TRANSCRIBER] Failed to set global undici dispatcher:', err.message);
}
