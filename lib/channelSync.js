...
/**
 * Ensures global track roles and track council channels exist in a guild.
				...
				if (memberObj) {
					...
					if (dbMember && dbMember.role) {
						...
						if (trackName && globalRoles[trackName]) {
							...
							if (!memberObj.roles.cache.has(trRole.id)) {
						...
						if (leadRoleName && globalRoles[leadRoleName]) {
							...
							if (!memberObj.roles.cache.has(ldRole.id)) {
