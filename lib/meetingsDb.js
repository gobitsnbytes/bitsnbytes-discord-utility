...
module.exports = {
	...
	async createMeeting(meeting) {
		if (!isTest) {
			...
			const response = await callMotherboard('POST', '/api/meetings/schedule', meeting.creatorId, {
				title: meeting.title,
	...
	async findMeetingByCalcomId(bookingId) {
		if (!isTest) {
			try {
				const { callMotherboard } = require('./motherboardApi');
				const list = await callMotherboard('GET', '/api/meetings', 'discord_bot');
				const matched = list.find(m => m.calcom_booking_id === bookingId || m.calcom_uid === bookingId);
				if (!matched) return null;
				return {
					...matched,
					scheduled_time: matched.scheduled_time,
