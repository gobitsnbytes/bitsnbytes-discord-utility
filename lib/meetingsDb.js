...
module.exports = {
	async createMeeting(meeting) {
		const { id, title, description, scheduledTime, locationType, locationDetails, creatorId } = meeting;
		const createdAt = Date.now();
		await dbRun(
			`INSERT INTO meetings (id, title, description, scheduled_time, location_type, location_details, status, creator_id, created_at)
			 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`,
			[id, title, description, scheduledTime, locationType, locationDetails, 'scheduled', creatorId, createdAt]
		);
		return id;
	},
